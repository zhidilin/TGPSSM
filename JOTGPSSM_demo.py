# main function of running (JO)TGPSSM
...
import time
import math
from module.TGPSSMs import JOTGPSSM_E as TGPSSM
...
# observation_noise_sd = math.sqrt(0.1)
...
dir = f'fig/fig_newFlow/{model._get_name()}/{func}_flBlock{num_flow_blocks}_changeOrder_learnNoise/obser_noise_std_{observation_noise_sd}/'
if not os.path.exists(dir):
    os.makedirs(dir)

lr = 0.005
optimizer = torch.optim.Adam(model.parameters(), lr=lr)
# transition_params = list(map(id, model.transition.parameters()))
# base_params = filter(lambda p: id(p) not in transition_params, model.parameters())
# optimizer = torch.optim.SGD([
#             {'params': base_params},
#             {'params': model.transition.parameters(), 'lr': lr/100}], lr=lr, momentum=0.9)
...
for epoch in epochiter:
    ...
    if epoch % 200 == 0: # plot the results
        _ = plot_1D_results(model=model, epoch=epoch, func=func, save=save_fig, path=dir)
